...
-- ==========================================================
-- Atualiza fqbn e port no sketch.yaml sem apagar baud
-- ==========================================================
local function UpdateSketchConfig()
    ...
    -- Detecta porta (ex: COM3)
    local port = output:match("(COM%d+)") or "COM3"

    -- Detecta FQBN (ex: arduino:avr:uno)
    local fqbn = output:match("(%S+:%S+:%S+)") or "arduino:avr:uno"

    -- Lê conteúdo existente (para preservar baud e comentários)
    local old_lines = {}
    ...
    -- Atualiza ou adiciona fqbn e port, mantendo o resto intacto
    local new_lines = {}
    local fqbn_written, port_written = false, false

    for _, line in ipairs(old_lines) do
        if line:match("^default_fqbn:") then
            table.insert(new_lines, "default_fqbn: " .. fqbn)
            fqbn_written = true
        elseif line:match("^default_port:") then
    ...
    -- Adiciona fqbn e port caso não existam
    if not fqbn_written then table.insert(new_lines, "fqbn: " .. fqbn) end
    if not port_written then table.insert(new_lines, "port: " .. port) end

    -- Escreve de volta preservando o restante
    ...
    vim.notify("✅ sketch.yaml atualizado com sucesso!\nFQBN: " .. fqbn .. "\nPORT: " .. port)
end
...
map_keys("n", "<F5>", function() UpdateSketchConfig(); CreateMakefile()
end, { desc = "Atualizar sketch.yaml e Makefile" })
