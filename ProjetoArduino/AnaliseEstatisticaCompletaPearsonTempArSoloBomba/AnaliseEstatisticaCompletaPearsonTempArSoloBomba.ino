...
#define RELE_PIN 7           // Pino para controle do relé da irrigação
#define TEMPO_IRRIGA_BASE 30000 // 30 segundos base

// ================= VARIÁVEIS GLOBAIS DE IRRIGAÇÃO =============================== 
...
/**
 * @struct Estatisticas @brief Armazena métricas estatísticas calculadas
 */
struct Estatisticas {
...
void logData(DadosSensores dados, Estatisticas stats_umid_ar, 
  ...
  String nomeArquivo = gerarNomeArquivo(tempoDecorrido);

  //  Abre o arquivo específico do dia
  dataFile = SD.open(nomeArquivo, FILE_WRITE);

  if (dataFile) {

    // Verifica se é um arquivo novo (cabeçalho)
    if (dataFile.size() == 0) {
      dataFile.println(F("Data,Hora,UmidadeAr,MedUAr,DesvUAr,Q1UAr,MedUAr,Q3UAr,OutUAr,Temp,MedT,DesvT,Q1T,MedT,Q3T,OutT,USolo,MedUS,DesvUS,Q1US,MedUS,Q3US,OutUS,Corr1,Corr2,Corr3,Irrigacao"));
    ...
    // Irrigação 
    dataFile.print(irrigando ? F("ATIVA") : F("INATIVA"));
...
// ================= FUNÇÃO DE IRRIGAÇÃO ==========================
void controlarIrrigacao(float umidadeAtual, Estatisticas stats) {
  unsigned long agora = millis();
  
  // Lógica simplificada baseada nos quartis
  if (!irrigando && umidadeAtual < stats.q1 && 
      (agora - inicioIrrigacao > 3600000)) { // 1h entre irrigações
    irrigando = true;
    inicioIrrigacao = agora;
    digitalWrite(RELE_PIN, LOW);
  }
  else if (irrigando && (agora - inicioIrrigacao > TEMPO_IRRIGA_BASE)) {
...
/** LEITURA DE ARQUIVO 
 * @brief Gera nome de arquivo baseado na data
 * @param timestamp Tempo em milissegundos
 * @return String no formato "AAAAMMDD.csv"
 */
String gerarNomeArquivo(unsigned long timestamp) {
  String data = calcularDataRelativa(timestamp);
  data.replace("-", ""); // Remove hifens
  return data + ".csv";
}
...
// ============ CONFIGURAÇÃO PARA IRRIGAÇÃO ===================
  pinMode(RELE_PIN, OUTPUT);
  digitalWrite(RELE_PIN, HIGH); // Inicia com relé desligado
...
void loop() {
  ...
  if (millis() - last_log >= LOG_INTERVAL) {
    ...
    if (!isnan(dados.umidade_ar) && !isnan(dados.temperatura)) {
      ...
      // Irrigação 
      controlarIrrigacao(dados.umidade_solo_percent, stats_umid_solo);
