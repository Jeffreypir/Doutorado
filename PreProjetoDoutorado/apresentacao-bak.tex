...
\begin{frame}{Atualização de Medição}
    ...
    \begin{itemize} 
        ...
        \item Normalização dada por: $\eta = p(y)^{-1}$.
    \end{itemize}
\end{frame}
...
\begin{frame}{Prototipagem com Arduino}
    \begin{block}{}
        Desenvolver e implementar protótipos utilizando a plataforma Arduino para aplicar robótica probabilística em diferentes contextos técnicos e sociais.
    \end{block}

    \begin{block}{}
        \begin{itemize}
            \item Construção de protótipos robóticos para aplicação prática.
            \item Implementação de algoritmos estocásticos para controle e navegação dos robôs.
            \item Testes em cenários diversos para avaliar a adaptabilidade e eficiência dos robôs.
        \end{itemize}
    \end{block}
...
% Estilos de nós
\tikzstyle{startstop} = [rectangle, rounded corners, minimum width=1.8cm, minimum height=0.6cm, text centered, draw=black, fill=red!30, font=\tiny]
\tikzstyle{process} = [rectangle, minimum width=1.8cm, minimum height=0.6cm, text centered, draw=black, fill=blue!30, font=\tiny]
\tikzstyle{decision} = [diamond, minimum width=2cm, minimum height=0.6cm, text centered, draw=black, fill=green!30, font=\tiny]
...
\begin{frame}{Publicações e Impacto}
    \begin{itemize}
        ...
        \item Partipação de feiras cientificas com aplicações da pesquisa no ensino médio.
    \end{itemize}
\end{frame}
