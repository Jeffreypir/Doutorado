...
\usepackage{amsmath}
\usepackage[ruled,vlined]{algorithm2e} % Removido o pacote 'algorithm' conflitante
\usepackage{graphicx}
\usepackage{lipsum}
\usepackage{tikz}
...
\begin{frame}{Introdução}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item Projetos com Arduino e robótica probabilística promovem inovação na educação e prototipagem de robôs autônomos.
        \end{itemize}
    \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]{Pseudocódigo do Algoritmo}
\begin{algorithm}[H]
\caption{Algoritmo FilterBayes}
\KwIn{$bel(x_{t-1}), u_t, z_t$}
\KwOut{$bel(x_t)$}
\For{todo $x_t$}{
    \textcolor{blue}{Previsão:} $\overline{bel}(x_t) \gets \int p(x_t \mid u_t, x_{t-1}) \, bel(x_{t-1}) \, dx_{t-1}$\;
    \textcolor{blue}{Atualização:} $bel(x_t) \gets \eta \cdot p(z_t \mid x_t) \cdot \overline{bel}(x_t)$\;
}
\Return{$bel(x_t)$}
\end{algorithm}
\end{frame}
...
% Estilos TikZ
\tikzset{
    startstop/.style={rectangle, rounded corners, minimum width=1.8cm, minimum height=0.6cm, text centered, draw=black, fill=red!30, font=\tiny},
    process/.style={rectangle, minimum width=1.8cm, minimum height=0.6cm, text centered, draw=black, fill=blue!30, font=\tiny},
    decision/.style={diamond, minimum width=2cm, minimum height=0.6cm, text centered, draw=black, fill=green!30, font=\tiny},
    arrow/.style={->,>=stealth, thin}
}

\begin{frame}{Fluxograma: Aplicação do Filtro de Bayes}
\centering
\begin{tikzpicture}[node distance=1.2cm and 1.5cm]
\node (start) [startstop] {Início};
\node (dataCollect) [process, below=of start] {Coleta de dados\\(sensores Arduino)};
\node (localization) [process, below=of dataCollect] {Estimativa de localização\\(filtro de Bayes)};
\node (mapUpdate) [process, right=of localization, xshift=2.5cm] {Atualização do mapa};
\node (decision) [decision, below=of localization, yshift=-0.3cm] {Plano necessário?};
...
\draw [arrow] (localization.east) -- (2.6,-2.4);
\draw [arrow] (localization) -- (decision);
\draw [arrow] (mapUpdate.south) |- (decision.east);
\draw [arrow] (decision.south) -- (motion.north);
...
\begin{frame}{Estrutura de um Robô}
...
\begin{frame}{Publicações e Impacto}
    \begin{itemize}
        ...
        \item Publicação de dois ou mais artigos em revistas especializadas em Estatística aplicada ou Robótica.
    \end{itemize}
\end{frame}
...
\begin{frame}{Wiki do projeto}
    \begin{center}
        \qrcode[height=5cm]{https://github.com/Jeffreypir/ProbabiliticRobotics/wiki/Rob\%C3\%B3tica-Probabilistica}
          ...
          {\scriptsize \textbf{Fonte:} Elaborado pelo autor.}
    \end{center}
\end{frame}
...
\begin{frame}{Agradeço a todos pela atenção!}
    \begin{center}
